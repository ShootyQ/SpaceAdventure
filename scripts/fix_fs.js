...
const manifest_content = `"use client";
...
const ManifestOverlay = memo(({ isVisible, onClose, ships, ranks, selectedIds, setSelectedIds, behaviors }: ManifestOverlayProps) => { // Removed Type Annotation for props in JS
                     ...
                     <AnimatePresence>
                        {selectedIds.size > 0 && (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2">
                                     ...
                                     <div className="relative group">
                                         <select 
                                            id="bulk-protocol"
                                            onChange={(e) => {
                                                const xpInput = document.getElementById('bulk-xp');
                                ...
                                <button 
                                    onClick={async () => {
                                        const xpInput = parseInt(document.getElementById('bulk-xp').value) || 0;
                                        const reasonInput = document.getElementById('bulk-reason').value || "Command Award";
                                        ...
                                        document.getElementById('bulk-xp').value = '';
...
const UpgradeSlot = ({ icon: Icon, label, level = 0, active = false }) => (
...
const UserAvatar = ({ userData, className = "" }) => {
...
function CockpitView({ onNavigate, ranks }) {
...
function ShipSettings({ userData, user }) {
...
function AvatarConfigView({ onBack }) {
    ...
    const [activeHat, setActiveHat] = useState('none');
    const [loading, setLoading] = useState(false);
...
function AvatarView({ onNavigate, ranks }) {
        ...
        <div className="min-h-screen bg-space-950 p-6 font-mono pb-20 overflow-x-hidden">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {view === 'cockpit' && <CockpitView onNavigate={(v) => setView(v)} ranks={ranks} />}
                        {view === 'ship' && <ShipSettings userData={userData} user={user} />}
                        {view === 'inventory' && <InventoryView />}
                        {view === 'avatar' && <AvatarView onNavigate={(v) => setView(v)} ranks={ranks} />}
...
try {
    fs.writeFileSync('components/ManifestOverlay.tsx', manifest_content);
    console.log("Successfully wrote components/ManifestOverlay.tsx");
} catch (e) {
    console.error("Error writing components/ManifestOverlay.tsx:", e);
}

try {
    fs.writeFileSync('app/student/settings/page.tsx', page_content);
    console.log("Successfully wrote app/student/settings/page.tsx");
} catch (e) {
    console.error("Error writing app/student/settings/page.tsx:", e);
}
